...
- The managed **Openship GitHub App private key** lives only on Openship Cloud. A self-hosted workspace may
  instead register one or more Apps in Settings; each private key, client secret, and webhook secret is
  stored in a versioned AES-256-GCM envelope and is never returned by the API. The legacy process-environment
  configuration remains supported. Every App path mints short-lived, repository-scoped installation tokens.
