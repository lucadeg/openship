...
Go to **Settings → Git** and choose a GitHub connection method. A GitHub window may open asking "do you allow
this?" — say yes. When it closes, you're connected.

<Callout title="Screenshot">Settings → Git with the GitHub connection choices. *(screenshot pending)*</Callout>

<Callout title="Self-hosted: two ways to connect (no Openship account required)" type="info">
On a self-hosted instance you're never forced through Openship Cloud. **Settings → Git** offers:
...
- **Your own self-hosted GitHub Apps** — use the one-click manifest flow or register one or more existing
  GitHub/GitHub Enterprise Apps in the dashboard, then use installation tokens without Openship Cloud or PATs. See
