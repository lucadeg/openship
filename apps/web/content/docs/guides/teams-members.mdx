...
Click **Invite member** in the top-right. An inline **Invite a member** panel expands on the page.
...
If the selected email transport is unavailable, the action changes to **Create invite link**. Create
it, then use **Copy** beside the pending invitation and send the link yourself.

<Callout title="Screenshot">The "Invite a member" panel: the email field, the Member / Admin / Restricted role cards, and the **Send invite** button. *(screenshot pending)*</Callout>
...
On self-hosted Openship, the instance administrator can create a brand-new user through this
invitation page. Workspace admins can invite users who already have an account, but they cannot mint
new instance accounts; ask the instance administrator to send the first invitation in that case.
...
ask an owner or admin to copy it from **Pending invitations** or send a fresh one.
</Callout>
...
but haven't set one up, switch the invite to **Openship Cloud** and send again. You can also use
**Create invite link**, then **Copy** it from the pending invitation and send it directly.
</Callout>
